...
Arei Xref backend provides =xref-buffer-location= in =arei-xref--make-xref-loc=
but Citre expects =xref-file-location=.  Advise the function.

#+begin_src elisp :noweb-ref emacs-init
  (use-package arei-xref
    :after citre
    :config
    (defun hako/arei-xref--make-xref-loc-advice (identifier)
    ...
    (advice-add #'arei-xref--make-xref-loc :override #'hako/arei-xref--make-xref-loc-advice)
...
flycheck-guile gets load paths from geiser.  Define a custom checker to use guix
repl instead.
...
#+begin_src elisp :noweb-ref emacs-init
  (use-package flycheck-guile
    :after flycheck
    :config
    (flycheck-define-checker guile-guix
      ...
      ("guild" "compile" "-O0"
       ...
       (eval (mapcan
              (lambda (p)
                (list "-L" p))
              (read (shell-command-to-string "echo '(write %load-path)' | guix repl /dev/stdin"))))
       source)
      ...
      ((warning
       ...
       (error
        ...
        (message (+? anything)) (* space) string-end))
      :modes (scheme-mode))
    (add-to-list 'flycheck-checkers 'guile-guix))
#+end_src
