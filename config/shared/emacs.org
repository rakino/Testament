...
*** packages (for manual installations)
#+begin_src elisp :noweb-ref emacs-init
  (use-package package
    :custom
    (package-archives
...
*** [[https://github.com/ArthurHeymans/emacs-tramp-rpc][tramp-rpc]]
...
#+begin_src elisp :noweb-ref emacs-init
  (defun hako/load-font ()
    (let* ((size     16)
           ...
           (modeline (font-spec :family "Chiron Sung HK VF" :size size))
...
#+begin_src elisp :noweb-ref emacs-init
  (use-package scheme
    ...
    (defun hako/adjust-scheme-indentation ()
      ...
      ;;; Rosenthal
      (put 'match-record/dolly                'scheme-indent-function 3)
      (put 'match-record-lambda/dolly         'scheme-indent-function 2)
