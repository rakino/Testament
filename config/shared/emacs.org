...
#+begin_src emacs-lisp :noweb-ref emacs-init
  (use-package magit
    :autoload (magit-toplevel)
    :custom
    (magit-define-global-key-bindings nil))

  (use-package majutsu
    :autoload (majutsu-toplevel)
    :init
    ;; Use `majutsu' for Jujutsu repositories and `magit' otherwise.
    (defun hako/majutsu-or-magit (&optional directory)
      (interactive
       (list (and (or current-prefix-arg
                      (not (or (majutsu-toplevel)
                               (magit-toplevel))))
      ...
      (let ((default-directory (or directory default-directory)))
        (if (majutsu-toplevel default-directory)
            (majutsu default-directory)
          (magit default-directory))))
    ;; `hako/majutsu-or-magit' for `project.el'.
    (defun hako/majutsu-or-magit-project ()
      (interactive)
      (when (fboundp 'project-root)
        (let ((directory (project-root (project-current t))))
          (hako/majutsu-or-magit directory))))
    ...
    :bind
    ("C-x g" . hako/majutsu-or-magit))

  (use-package project
    :init
    :config
    (add-to-list 'project-switch-commands
                 '(hako/majutsu-or-magit-project "Majutsu or Magit")
                 t)
    :bind
    (:map project-prefix-map
          ("m" . hako/majutsu-or-magit-project)))
#+end_src
