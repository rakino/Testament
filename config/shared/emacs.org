...
Xref stops searching if one backend fails to find the symbol.  Replacing it with
...
Citre has builtin support for Eglot backend.  Other backends will be converted
later in their own sections.
...
#+begin_src emacs-lisp :noweb-ref emacs-init
  (use-package eglot
    ...
    :hook
    ((c-mode python-mode rust-ts-mode zig-mode) . eglot-ensure))
#+end_src
