...
#+begin_src nix :tangle ../tangled/dorphine/nixpkgs.nix
  (import (builtins.getFlake "github:NixOS/nixpkgs/714a5f8c4ead6b31148d829288440ed033ccc041") {})
#+end_src
...
Create /etc/fonts/fonts.conf to specify font directories and extra
configurations, see also my previous extensions to home-fontconfig-service-type.
...
NixOS installs graphics drivers to /run/opengl-driver, when using Nix packages
we'll need this as well, since GLX and CUDA drivers don't have search paths.
Alternatively, you can load them (libGL.so, libcuda.so, libnvcuvid.so) via the
LD_PRELOAD environment variable.

Even if you're not using a NVIDIA graphics card, add the nvda package from
Nonguix to system profile as well (not needed for NVIDIA cards though, since
this has already been handled by nonguix-transformation-nvidia properly).  That
package provides a mesa variant with GLVND support and search paths for various
graphics libraries.
